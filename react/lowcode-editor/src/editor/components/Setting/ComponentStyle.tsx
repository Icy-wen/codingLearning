...
export default function ComponentStyle() {
  ...
  const { curComponentId, curComponent, updateComponentsStyle } = useComponentsStore();
  ...
  function valueChange(changeValues: CSSProperties) {
    if (curComponentId) {
      updateComponentsStyle(curComponentId, changeValues);
    }
  }
  ...
  const handleEditorChange = debounce((value) => {
    ...
    try {
      ...
      updateComponentsStyle(curComponentId, { ...form.getFieldsValue(), ...css }, true);
    } catch (e) { }
  }, 500);
  ...
  return (
    ...
    >
      {
        componentsConfig[curComponent.name]?.stylesSetter?.map(setter => (
