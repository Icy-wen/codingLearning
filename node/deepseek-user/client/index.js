...
window.onload = async function () {
    const users = await fetchUser();//调用fetchUser函数，获取用户数据
    renderUser(users);
}
async function fetchUser(){
    //向后端发送请求，获取用户数据
    const res= await fetch(`http://localhost:3000/api/users`);//接口地址
...
btn.addEventListener('click', async function () {//添加点击事件
    const users = await searchUser();//调用searchUser函数，获取用户数据
